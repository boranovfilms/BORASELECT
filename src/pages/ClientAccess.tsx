    ...
    <div className=\"space-y-12 pb-20 text-left\">
      {!isClientFormOpen && !isTeamViewOpen && (
        <header className=\"flex flex-col md:flex-row md:items-end justify-between gap-6\">
          <div className=\"text-left\"><h1 className=\"text-5xl font-bold tracking-tight text-white mb-2 uppercase italic font-black\">Gestão de Clientes</h1><p className=\"text-zinc-500 text-lg\">Gerencie o cadastro dos seus clientes.</p></div>
          ...
          <div className=\"space-y-6\">
            <section className=\"bg-[#1f1f1f] border border-zinc-800/80 rounded-3xl p-6 shadow-xl text-left\">
              ...
              <div className=\"flex flex-col md:flex-row gap-8\">
                <div className=\"space-y-2\"><label className=\"text-[10px] uppercase font-black tracking-widest text-zinc-500 block ml-1\">Logo do Cliente</label><button type=\"button\" onClick={() => fileInputRef.current?.click()} className=\"w-[150px] h-[150px] rounded-2xl border-2 border-dashed border-zinc-800 bg-zinc-900/50 hover:bg-zinc-900 transition-all flex flex-col items-center justify-center gap-2 group overflow-hidden relative shadow-inner\">{logoPreview ? <img src={logoPreview} alt=\"Logo\" className=\"w-full h-full object-cover\" /> : (clientForm.logoUrl ? <img src={clientForm.logoUrl} alt=\"Logo\" className=\"w-full h-full object-cover\" /> : <><Upload className=\"w-6 h-6 text-zinc-600 group-hover:text-[#ff5351]\" /><span className=\"text-[9px] font-black uppercase text-zinc-500\">300x300px</span></>)}</button><input type=\"file\" ref={fileInputRef} className=\"hidden\" accept=\"image/*\" onChange={handleLogoSelect} /></div>
                <div className=\"flex-1 space-y-5\"><div className=\"grid grid-cols-1 md:grid-cols-2 gap-5\"><div className=\"space-y-2\"><label className=\"text-[10px] uppercase font-black tracking-widest text-zinc-500\">Nome da Empresa</label><input type=\"text\" value={clientForm.name} onChange={e => setClientForm({...clientForm, name: e.target.value.toUpperCase()})} className=\"w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-4 text-white focus:border-[#ff5351] outline-none text-sm uppercase font-bold\" placeholder=\"NOME COMERCIAL\"/></div><div className=\"space-y-2\"><label className=\"text-[10px] uppercase font-black tracking-widest text-zinc-500\">E-MAIL PRINCIPAL</label><input type=\"email\" value={clientForm.email} onChange={e => setClientForm({...clientForm, email: e.target.value.toLowerCase().trim()})} disabled={isEditingClient} className=\"w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-4 text-white focus:border-[#ff5351] outline-none text-sm disabled:opacity-50 font-bold\" placeholder=\"contato@empresa.com\"/></div></div><div className=\"grid grid-cols-1 md:grid-cols-2 gap-5\"><div className=\"space-y-2\"><label className=\"text-[10px] uppercase font-black tracking-widest text-zinc-500\">Razão Social / CNPJ</label><div className=\"relative\"><input type=\"text\" value={clientForm.document} onChange={e => setClientForm({...clientForm, document: e.target.value.toUpperCase()})} className=\"w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-4 pr-12 text-white focus:border-[#ff5351] outline-none text-sm uppercase font-bold\" placeholder=\"NOME LEGAL OU CNPJ\"/><button type=\"button\" onClick={handleFetchCnpj} className=\"absolute right-4 top-1/2 -translate-y-1/2 text-zinc-500 hover:text-[#ff5351] transition-colors\">{searchingCnpj ? <Loader2 className=\"w-4 h-4 animate-spin\" /> : <Search className=\"w-4 h-4\" />}</button></div></div><div className=\"space-y-2\"><label className=\"text-[10px] uppercase font-black tracking-widest text-zinc-500\">Site / Link</label><div className=\"relative\"><input type=\"text\" value={clientForm.website} onChange={e => setClientForm({...clientForm, website: e.target.value.toLowerCase().trim()})} className=\"w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-4 pl-11 text-white focus:border-[#ff5351] outline-none text-sm font-bold\" placeholder=\"www.empresa.com.br\"/><Globe className=\"w-4 h-4 text-zinc-700 absolute left-4 top-1/2 -translate-y-1/2\" /></div></div></div></div>
              </div>
            </section>
            <section className=\"bg-[#1f1f1f] border border-zinc-800/80 rounded-3xl p-6 shadow-xl text-left\"><h3 className=\"text-lg font-bold text-white mb-5 flex items-center gap-2\"><MapPin className=\"w-5 h-5 text-[#ff5351]\"/> Endereço</h3><div className=\"grid grid-cols-1 md:grid-cols-4 gap-5 mb-4\"><div className=\"space-y-2\"><label className=\"text-[10px] uppercase font-black tracking-widest text-zinc-500\">CEP</label><input type=\"text\" value={clientForm.zipCode} onChange={e => setClientForm({...clientForm, zipCode: e.target.value})} className=\"w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-white focus:border-[#ff5351] outline-none text-sm font-bold\" placeholder=\"00000-000\"/></div><div className=\"space-y-2 md:col-span-3\"><label className=\"text-[10px] uppercase font-black tracking-widest text-zinc-500 uppercase\">Rua / Logradouro</label><input type=\"text\" value={clientForm.address} onChange={e => setClientForm({...clientForm, address: e.target.value.toUpperCase()})} className=\"w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-white focus:border-[#ff5351] outline-none text-sm uppercase font-bold\" placeholder=\"AV. PRINCIPAL\"/></div></div><div className=\"grid grid-cols-1 md:grid-cols-4 gap-5\"><div className=\"space-y-2\"><label className=\"text-[10px] uppercase font-black tracking-widest text-zinc-500 uppercase\">Número</label><input type=\"text\" value={clientForm.number} onChange={e => setClientForm({...clientForm, number: e.target.value.toUpperCase()})} className=\"w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-white focus:border-[#ff5351] outline-none text-sm uppercase font-bold\" placeholder=\"123\"/></div><div className=\"space-y-2\"><label className=\"text-[10px] uppercase font-black tracking-widest text-zinc-500 uppercase\">Bairro</label><input type=\"text\" value={clientForm.neighborhood} onChange={e => setClientForm({...clientForm, neighborhood: e.target.value.toUpperCase()})} className=\"w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-white focus:border-[#ff5351] outline-none text-sm uppercase font-bold\" placeholder=\"CENTRO\"/></div><div className=\"space-y-2\"><label className=\"text-[10px] uppercase font-black tracking-widest text-zinc-500 uppercase\">Cidade</label><input type=\"text\" value={clientForm.city} onChange={e => setClientForm({...clientForm, city: e.target.value.toUpperCase()})} className=\"w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-white focus:border-[#ff5351] outline-none text-sm uppercase font-bold\" placeholder=\"SÃO PAULO\"/></div><div className=\"space-y-2\"><label className=\"text-[10px] uppercase font-black tracking-widest text-zinc-500 uppercase\">Estado</label><input type=\"text\" value={clientForm.state} onChange={e => setClientForm({...clientForm, state: e.target.value.toUpperCase()})} className=\"w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-white focus:border-[#ff5351] outline-none text-sm uppercase font-bold\" placeholder=\"SP\"/></div></div></section>
            <section className=\"bg-[#1f1f1f] border border-zinc-800/80 rounded-3xl p-6 shadow-xl mb-10 text-left\"><h3 className=\"text-lg font-bold text-white mb-5 flex items-center gap-2 uppercase italic leading-none\"><Briefcase className=\"w-5 h-5 text-[#ff5351]\"/> Dados Comerciais</h3><div className=\"grid grid-cols-1 md:grid-cols-2 gap-5\"><div className=\"space-y-2\"><label className=\"text-[10px] uppercase font-black tracking-widest text-zinc-500\">Telefone / WhatsApp</label><div className=\"relative\"><input type=\"text\" value={clientForm.phone} onChange={e => setClientForm({...clientForm, phone: e.target.value})} className=\"w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-4 pl-11 text-white text-sm font-bold\" placeholder=\"(00) 00000-0000\"/><Phone className=\"w-4 h-4 text-zinc-700 absolute left-4 top-1/2 -translate-y-1/2\" /></div></div><div className=\"space-y-2\"><label className=\"text-[10px] uppercase font-black tracking-widest text-zinc-500 uppercase\">Responsável de Contato</label><div className=\"relative\"><input type=\"text\" value={clientForm.responsibleContact} onChange={e => setClientForm({...clientForm, responsibleContact: e.target.value.toUpperCase()})} className=\"w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-4 pl-11 text-white focus:border-[#ff5351] outline-none text-sm uppercase font-bold\" placeholder=\"NOME DO RESPONSÁVEL LEGAL\"/><User className=\"w-4 h-4 text-zinc-700 absolute left-4 top-1/2 -translate-y-1/2\" /></div></div></div></section>
          </div>
        </div>
      )}
      ...
      {isTeamViewOpen && selectedClient && (
        <div className=\"animate-in slide-in-from-bottom-8 duration-500 text-left\">
          <header className=\"flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8 border-b border-zinc-800 pb-6\">
            <div className=\"text-left\"><button onClick={() => setIsTeamViewOpen(false)} className=\"text-[#ff5351] hover:text-white transition-colors text-sm font-bold flex items-center gap-2 mb-4 uppercase tracking-widest leading-none\"><ChevronDown className=\"w-4 h-4 rotate-90\" /> Voltar</button><h1 className=\"text-4xl font-bold tracking-tight text-white mb-2 uppercase italic font-black leading-tight text-left\">Equipe: {selectedClient.name}</h1></div>
          ...
          {isAddingTeamMember && (
            <form autoComplete=\"off\" onSubmit={handleSaveTeamMember} className=\"space-y-6 mb-12 animate-in fade-in duration-300 text-left\">
              <section className=\"bg-[#1f1f1f] border border-zinc-800/80 rounded-3xl p-8 shadow-xl text-left\">
                <div className=\"flex items-center gap-3 mb-8 italic uppercase font-black tracking-widest text-[#ff5351]\"><User className=\"w-5 h-5\" /><h3>Informações Pessoais</h3></div>
                <div className=\"flex flex-col md:flex-row gap-10 items-start\">
                  <div className=\"space-y-3 text-center\">
                    <label className=\"text-[10px] uppercase font-black tracking-widest text-zinc-500 block text-center leading-none\">Foto do Membro</label>
                    <button type=\"button\" onClick={() => memberFileInputRef.current?.click()} className=\"w-[110px] h-[110px] rounded-full border-2 border-dashed border-zinc-800 bg-zinc-900/50 hover:bg-zinc-900 hover:border-[#ff5351]/50 transition-all flex flex-col items-center justify-center gap-1 group overflow-hidden relative shadow-inner\">
                      {memberForm.photoUrl ? (
                        ...
                        <>
                          ...
                          <span className=\"text-[8px] font-black uppercase text-zinc-500\">2MB MÁX</span></>
                      )}
                    </button>
                    {memberForm.photoUrl && (
              ...
              <section className=\"bg-[#1f1f1f] border border-zinc-800/80 rounded-3xl p-8 shadow-xl text-left\">
                <div className=\"flex items-center gap-3 mb-8 text-left\"><Briefcase className=\"w-5 h-5 text-[#ff5351]\" /><h3 className=\"text-lg font-bold text-white uppercase italic leading-none\">Informações Profissionais</h3></div>
                ...
                <div className=\"space-y-3 text-left\"><label className=\"text-[10px] uppercase font-black tracking-widest text-zinc-500 block text-left\">NÍVEL DE ACESSO</label><div className=\"grid grid-cols-1 md:grid-cols-3 gap-4\">\n                    {[\n                      { id: 'aprovador', label: 'Aprovador Principal', desc: 'Aprovação final do conteúdo' },\n                      { id: 'validador', label: 'Validador', desc: 'Sugere correções no conteúdo' },\n                      { id: 'visualizador', label: 'Visualizador', desc: 'Apenas acompanha o projeto' }\n                    ].map(level => (\n                      <label key={level.id} className={cn(\"cursor-pointer border rounded-2xl p-5 flex items-start gap-3 transition-all\", memberForm.accessLevel === level.id ? \"bg-[#ff5351]/10 border-[#ff5351]\" : \"bg-zinc-900 border-zinc-800 hover:border-zinc-300\")}><div className=\"pt-0.5\"><div className={cn(\"w-4 h-4 rounded-full border-2 flex items-center justify-center transition-all\", memberForm.accessLevel === level.id ? \"border-[#ff5351]\" : \"border-zinc-600\")}>{memberForm.accessLevel === level.id && <div className=\"w-2 h-2 rounded-full bg-[#ff5351]\" />}</div></div><div className=\"text-left\"><input type=\"radio\" name=\"accessLevel\" value={level.id} checked={memberForm.accessLevel === level.id} onChange={e => setMemberForm({...memberForm, accessLevel: e.target.value})} className=\"hidden\" /><p className=\"text-white font-bold text-sm uppercase leading-none\">{level.label}</p><p className=\"text-zinc-500 text-[10px] mt-2 uppercase leading-tight\">{level.desc}</p></div></label>\n                    ))}\n                  </div></div>\n              </section>\n              <div className=\"flex justify-end pt-4\"><button type=\"submit\" disabled={loading} className=\"px-10 py-5 bg-white text-black font-black uppercase tracking-widest text-xs rounded-2xl hover:bg-[#ff5351] hover:text-white transition-all shadow-2xl disabled:opacity-50 flex items-center gap-3\"><UserPlus className=\"w-5 h-5\" />{loading ? 'Aguarde...' : 'Salvar Novo Membro'}</button></div>\n            </form>\n          )}\n          <DataTable data={allTeamMembers.filter(m => (m as any).clienteId === selectedClient.id)} loading={loading} emptyMessage=\"Nenhum membro vinculado.\" columns={[{ header: 'Membro', accessor: (member) => <div className=\"text-left py-1\"><div className=\"text-white font-bold text-base leading-tight mb-1 uppercase\">{member.name}</div><div className=\"text-zinc-500 text-xs flex items-center gap-2 leading-none uppercase\"><Mail className=\"w-3.5 h-3.5\" />{member.email}</div></div> }, { header: 'Status', accessor: (member) => <span className={cn(\"inline-flex items-center gap-2 px-3 py-1 border rounded-full text-[10px] font-black uppercase tracking-widest\", member.status === 'confirmed' ? \"text-emerald-400 border-emerald-500/30 bg-emerald-500/10\" : \"text-amber-400 border-amber-500/30 bg-amber-500/10\")}><div className={cn(\"w-1.5 h-1.5 rounded-full shadow-[0_0_8px_#10b981]\", member.status === 'confirmed' ? \"bg-emerald-500\" : \"bg-amber-500\")} />{member.status === 'confirmed' ? '✓ OK, SENHA CRIADA' : '⏳ AGUARDANDO SENHA'}</span> }]} actions={(member) => (<><button onClick={(e) => { e.stopPropagation(); sendEmailInvite(member.name, member.email); }} disabled={sendingEmail === member.email} className=\"p-2 bg-zinc-800 border border-zinc-700 hover:bg-[#ff5351] hover:border-transparent rounded-xl text-zinc-300 hover:text-white transition-all shadow-lg\" title=\"Reenviar Convite\"><Mail className=\"w-4 h-4\" /></button><button onClick={(e) => { e.stopPropagation(); handleDeleteTeamMember(member.id!, member.email); }} className=\"p-2 bg-zinc-800/50 hover:bg-red-500/10 rounded-xl text-zinc-600 hover:text-red-500 transition-all shadow-lg\" title=\"Excluir Membro\"><Trash2 className=\"w-4 h-4\" /></button></>)} />\n        </div>\n      )}\n\n      {!isClientFormOpen && !isTeamViewOpen && (\n        <div className=\"space-y-4 text-left\">\n          <div className=\"relative animate-in fade-in duration-300\"><Search className=\"w-5 h-5 text-zinc-500 absolute left-5 top-1/2 -translate-y-1/2\" /><input type=\"text\" placeholder=\"Buscar cliente por nome ou e-mail...\" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className=\"w-full bg-[#1a1a1a] border border-zinc-800 rounded-2xl pl-13 pr-5 py-4 text-white focus:border-[#ff5351] outline-none transition-all placeholder:text-zinc-600 shadow-xl uppercase font-medium text-left\"/></div>\n          <DataTable data={filteredClients} loading={loading} onRowClick={(client) => { if (client.id) navigate(`/clients/${client.id}`); }} emptyMessage={searchTerm ? 'Nenhum cliente encontrado.' : 'Nenhum cliente cadastrado.'} columns={[{ header: 'Cliente', accessor: (client) => (<div onClick={(e) => { e.stopPropagation(); if(client.id) navigate(`/clients/${client.id}`); }} className=\"text-left py-1 cursor-pointer group/name flex items-center gap-3\"><div className=\"w-10 h-10 rounded-xl bg-zinc-900 overflow-hidden border border-zinc-800 shrink-0 flex items-center justify-center shadow-lg\">{client.logoUrl ? <img src={client.logoUrl} alt=\"\" className=\"w-full h-full object-contain\" /> : <Building className=\"w-5 h-5 text-zinc-700\" />}</div><div className=\"text-left\"><div className=\"text-white font-black text-base leading-tight mb-1 uppercase italic group-hover/name:text-[#ff5351] transition-colors\">{client.name}</div><div className=\"text-zinc-500 text-[10px] font-black uppercase tracking-widest flex items-center gap-2\"><Mail className=\"w-3.5 h-3.5\" />{client.email}</div></div></div>) }, { header: 'Status', accessor: (client) => <span className={cn(\"inline-flex items-center gap-2 px-3 py-1 border rounded-full text-[10px] font-black uppercase tracking-widest\", client.status === 'confirmed' ? \"text-emerald-400 border-emerald-500/30 bg-emerald-500/10\" : \"text-amber-400 border-amber-500/30 bg-amber-500/10\")}><div className={cn(\"w-1.5 h-1.5 rounded-full shadow-[0_0_8px_#10b981]\", client.status === 'confirmed' ? \"bg-emerald-500\" : \"bg-amber-500\")} />{client.status === 'confirmed' ? '✓ OK, SENHA CRIADA' : '⏳ AGUARDANDO SENHA'}</span> }, { header: 'Membros', align: 'center', accessor: (client) => { const count = getClientTeamCount(client.id!); return count > 0 ? <div className=\"w-8 h-8 rounded-full bg-zinc-900 border border-zinc-800 flex items-center justify-center text-white text-[10px] font-black shadow-lg italic\">{count}</div> : <span className=\"text-zinc-700 text-xs font-medium\">-</span>; } }]} actions={(client) => (<><button onClick={(e) => { e.stopPropagation(); handleOpenTeamView(client); }} className=\"flex items-center gap-2 px-4 py-2 bg-zinc-800 border border-zinc-700 hover:bg-zinc-700 hover:border-transparent rounded-xl text-zinc-300 hover:text-white transition-all text-[10px] font-black uppercase tracking-widest shadow-lg\"><Users className=\"w-4 h-4 text-[#ff5351]\" />Equipe</button><button onClick={(e) => { e.stopPropagation(); handleOpenEditClient(client); }} className=\"p-2 bg-zinc-800 border border-zinc-700 hover:bg-zinc-700 rounded-xl text-zinc-400 hover:text-white transition-all shadow-lg\"><Edit2 className=\"w-4 h-4\" /></button><button onClick={(e) => { e.stopPropagation(); sendEmailInvite(client.name, client.email); }} disabled={sendingEmail === client.email} className=\"p-2 bg-zinc-800 border border-zinc-700 hover:bg-[#ff5351] hover:border-transparent rounded-xl text-zinc-300 hover:text-white transition-all disabled:opacity-50 shadow-lg\"><Mail className=\"w-4 h-4\" /></button><button onClick={(e) => { e.stopPropagation(); const inviteLink = `${window.location.origin}/register?email=${encodeURIComponent(client.email)}`; navigator.clipboard.writeText(`Seu conteúdo foi selecionado!\nAcesse pelo link abaixo e crie sua senha:\n\n${inviteLink}`); toast.success('Mensagem de convite copiada!'); }} className=\"p-2 bg-zinc-800 border border-zinc-700 hover:bg-[#25D366] hover:border-transparent rounded-xl text-zinc-300 hover:text-white transition-all shadow-lg\"><ExternalLink className=\"w-4 h-4\" /></button><button onClick={(e) => { e.stopPropagation(); handleDeleteClient(client.id!, client.email); }} className=\"p-2 bg-zinc-800/50 hover:bg-red-500/10 rounded-xl text-zinc-600 hover:text-red-500 transition-all shadow-lg\"><Trash2 className=\"w-4 h-4\" /></button></>)} />\n        </div>\n      )}\n    </div>\n  );\n}\n",path: