        ...
        <div className="animate-in slide-in-from-bottom-8 duration-500 text-left">
          ...
          {isAddingTeamMember && (
            <form autoComplete="off" onSubmit={handleSaveTeamMember} className="space-y-6 mb-12 animate-in fade-in duration-300 text-left">
              <section className="bg-[#1f1f1f] border border-zinc-800/80 rounded-3xl p-8 shadow-xl text-left">
                ...
                <div className="flex flex-col md:flex-row gap-10 items-start text-left">
                  <div className="space-y-3 text-left">
                    <label className="text-[10px] uppercase font-black tracking-widest text-zinc-500 block ml-1 text-left">Foto</label>
                    <button type="button" onClick={() => memberFileInputRef.current?.click()} className="w-[110px] h-[110px] rounded-full border-2 border-dashed border-zinc-800 bg-zinc-900/50 hover:bg-zinc-900 transition-all flex flex-col items-center justify-center gap-1 group overflow-hidden relative">
                      {memberForm.photoUrl ? (
                  ...
                  <div className="flex-1 space-y-6 w-full text-left">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5 text-left">
                      <div className="space-y-2 uppercase text-left"><label className="text-[10px] uppercase font-black tracking-widest text-zinc-500 block text-left text-left">NOME</label><input type="text" value={memberForm.firstName} onChange={e => setMemberForm({...memberForm, firstName: e.target.value.toUpperCase()})} placeholder="Ex: JOÃO" className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-4 text-white focus:border-[#ff5351] outline-none text-sm text-left uppercase font-bold text-left"/></div>
                      <div className="space-y-2 uppercase text-left"><label className="text-[10px] uppercase font-black tracking-widest text-zinc-500 block text-left text-left">SOBRENOME</label><input type="text" value={memberForm.lastName} onChange={e => setMemberForm({...memberForm, lastName: e.target.value.toUpperCase()})} placeholder="Ex: SILVA" className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-4 text-white focus:border-[#ff5351] outline-none text-sm text-left uppercase font-bold text-left"/></div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5 text-left">
                      <div className="space-y-2 text-left"><label className="text-[10px] uppercase font-black tracking-widest text-zinc-500 block text-left text-left">E-MAIL</label><div className="relative"><input type="email" value={memberForm.email} onChange={e => setMemberForm({...memberForm, email: e.target.value.toLowerCase().trim()})} placeholder="joao.silva@empresa.com" className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-4 pl-11 text-white focus:border-[#ff5351] outline-none text-sm text-left font-bold"/><Mail className="w-4 h-4 text-zinc-700 absolute left-4 top-1/2 -translate-y-1/2 text-left" /></div></div>
                      <div className="space-y-2 text-left"><label className="text-[10px] uppercase font-black tracking-widest text-zinc-500 block text-left text-left">TELEFONE</label><div className="relative"><input type="text" value={memberForm.phone} onChange={e => setMemberForm({...memberForm, phone: e.target.value})} placeholder="(00) 00000-0000" className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-4 pl-11 text-white focus:border-[#ff5351] outline-none text-sm text-left font-bold"/><Phone className="w-4 h-4 text-zinc-700 absolute left-4 top-1/2 -translate-y-1/2 text-left" /></div></div>
                    </div>
                  </div>
                </div>
              </section>
              ...
              <section className="bg-[#1f1f1f] border border-zinc-800/80 rounded-3xl p-8 shadow-xl text-left">
                <div className="flex items-center gap-3 mb-8 text-left"><Briefcase className="w-5 h-5 text-[#ff5351]" /><h3 className="text-lg font-bold text-white uppercase italic text-left">Informações Profissionais</h3></div>
          ...
          <DataTable 
            ...
            columns={[
              { header: 'Membro', accessor: (member) => <div className="text-left py-1 text-left"><div className="text-white font-bold text-base leading-tight mb-1 uppercase text-left">{member.name}</div><div className="text-zinc-500 text-xs flex items-center gap-2 leading-none uppercase text-left text-left"><Mail className="w-3.5 h-3.5 text-left" />{member.email}</div></div> },
            ...
            actions={(member) => (
              <>
                <button onClick={(e) => { e.stopPropagation(); sendEmailInvite(member.name, member.email); }} disabled={sendingEmail === member.email} className="p-2 bg-zinc-800 border border-zinc-700 hover:bg-[#ff5351] hover:border-transparent rounded-xl text-zinc-300 hover:text-white transition-all shadow-lg text-left" title="Reenviar Convite"><Mail className="w-4 h-4 text-left" /></button>
                <button onClick={(e) => { e.stopPropagation(); handleDeleteTeamMember(member.id!, member.email); }} className="p-2 bg-zinc-800/50 hover:bg-red-500/10 rounded-xl text-zinc-600 hover:text-red-500 transition-all shadow-lg text-left" title="Excluir Membro"><Trash2 className="w-4 h-4 text-left" /></button>
              </>
            )}
          />
        </div>
      ...
      {!isClientFormOpen && !isTeamViewOpen && (
        <div className="space-y-4 text-left">
          <div className="relative animate-in fade-in duration-300">
            ...
            <input type="text" placeholder="Buscar cliente por nome ou e-mail..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full bg-[#1a1a1a] border border-zinc-800 rounded-2xl pl-13 pr-5 py-4 text-white focus:border-[#ff5351] outline-none transition-all placeholder:text-zinc-600 shadow-xl uppercase font-medium text-left"/>
          </div>
          ...
          <DataTable 
                ...
                accessor: (client) => (
                  <div onClick={(e) => { e.stopPropagation(); if(client.id) navigate(`/clients/${client.id}`); }} className="text-left py-1 cursor-pointer group/name flex items-center gap-3">
                    ...
                    <div className="text-left">
                      <div className="text-white font-black text-base leading-tight mb-1 uppercase italic group-hover/name:text-[#ff5351] transition-colors text-left">{client.name}</div>
                      <div className="text-zinc-500 text-[10px] font-black uppercase tracking-widest flex items-center gap-2 text-left"><Mail className="w-3.5 h-3.5 text-left" />{client.email}</div>
                    </div>
                  </div>
                )
              },
            ...
            actions={(client) => (
              <>
                <button onClick={(e) => { e.stopPropagation(); handleOpenTeamView(client); }} className="flex items-center gap-2 px-4 py-2 bg-zinc-800 border border-zinc-700 hover:bg-zinc-700 hover:border-transparent rounded-xl text-zinc-300 hover:text-white transition-all text-[10px] font-black uppercase tracking-widest shadow-lg text-left"><Users className="w-4 h-4 text-[#ff5351] text-left" />Equipe</button>
                <button onClick={(e) => { e.stopPropagation(); handleOpenEditClient(client); }} className="p-2 bg-zinc-800 border border-zinc-700 hover:bg-zinc-700 rounded-xl text-zinc-400 hover:text-white transition-all shadow-lg text-left"><Edit2 className="w-4 h-4 text-left" /></button>
                <button onClick={(e) => { e.stopPropagation(); sendEmailInvite(client.name, client.email); }} disabled={sendingEmail === client.email} className="p-2 bg-zinc-800 border border-zinc-700 hover:bg-[#ff5351] hover:border-transparent rounded-xl text-zinc-300 hover:text-white transition-all disabled:opacity-50 shadow-lg text-left"><Mail className="w-4 h-4 text-left" /></button>
                <button onClick={(e) => { e.stopPropagation(); const inviteLink = `${window.location.origin}/register?email=${encodeURIComponent(client.email)}`; navigator.clipboard.writeText(`Seu conteúdo foi selecionado!\nAcesse pelo link abaixo e crie sua senha:\n\n${inviteLink}`); toast.success('Mensagem de convite copiada!'); }} className="p-2 bg-zinc-800 border border-zinc-700 hover:bg-[#25D366] hover:border-transparent rounded-xl text-zinc-300 hover:text-white transition-all shadow-lg text-left"><ExternalLink className="w-4 h-4 text-left" /></button>
                <button onClick={(e) => { e.stopPropagation(); handleDeleteClient(client.id!, client.email); }} className="p-2 bg-zinc-800/50 hover:bg-red-500/10 rounded-xl text-zinc-600 hover:text-red-500 transition-all shadow-lg text-left"><Trash2 className="w-4 h-4 text-left" /></button>
              </>
            )}
          />
        </div>
