          ...
          <div className="space-y-6">
            <section className="bg-[#1f1f1f] border border-zinc-800/80 rounded-3xl p-6 shadow-xl text-left">
              ...
              <div className="flex flex-col md:flex-row gap-8">
                <div className="space-y-2"><label className="text-[10px] uppercase font-black tracking-widest text-zinc-500 block ml-1">Logo do Cliente</label><button type="button" onClick={() => fileInputRef.current?.click()} className="w-[150px] h-[150px] rounded-2xl border-2 border-dashed border-zinc-800 bg-zinc-900/50 hover:bg-zinc-900 transition-all flex flex-col items-center justify-center gap-2 group overflow-hidden relative">{logoPreview ? <img src={logoPreview} alt="Logo" className="w-full h-full object-cover" /> : <><Upload className="w-6 h-6 text-zinc-600 group-hover:text-[#ff5351]" /><span className="text-[9px] font-black uppercase text-zinc-500">300x300px</span></>}</button><input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={handleLogoSelect} /></div>
                <div className="flex-1 space-y-5"><div className="grid grid-cols-1 md:grid-cols-2 gap-5"><div className="space-y-2"><label className="text-[10px] uppercase font-black tracking-widest text-zinc-500">Nome da Empresa</label><input type="text" value={clientForm.name} onChange={e => setClientForm({...clientForm, name: e.target.value.toUpperCase()})} className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-4 text-white focus:border-[#ff5351] outline-none text-sm uppercase" placeholder="NOME COMERCIAL"/></div><div className="space-y-2"><label className="text-[10px] uppercase font-black tracking-widest text-zinc-500">E-MAIL PRINCIPAL</label><input type="email" value={clientForm.email} onChange={e => setClientForm({...clientForm, email: e.target.value.toLowerCase().trim()})} disabled={isEditingClient} className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-4 text-white focus:border-[#ff5351] outline-none text-sm disabled:opacity-50" placeholder="contato@empresa.com"/></div></div><div className="grid grid-cols-1 md:grid-cols-2 gap-5"><div className="space-y-2"><label className="text-[10px] uppercase font-black tracking-widest text-zinc-500">Razão Social / CNPJ</label><div className="relative"><input type="text" value={clientForm.document} onChange={e => setClientForm({...clientForm, document: e.target.value.toUpperCase()})} className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-4 pr-12 text-white focus:border-[#ff5351] outline-none text-sm uppercase" placeholder="NOME LEGAL OU CNPJ"/><button type="button" onClick={handleFetchCnpj} className="absolute right-4 top-1/2 -translate-y-1/2 text-zinc-500 hover:text-[#ff5351] transition-colors">{searchingCnpj ? <Loader2 className="w-4 h-4 animate-spin" /> : <Search className="w-4 h-4" />}</button></div></div><div className="space-y-2"><label className="text-[10px] uppercase font-black tracking-widest text-zinc-500">Site / Link</label><div className="relative"><input type="text" value={clientForm.website} onChange={e => setClientForm({...clientForm, website: e.target.value.toLowerCase().trim()})} className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-4 pl-11 text-white focus:border-[#ff5351] outline-none text-sm" placeholder="www.empresa.com.br"/><Globe className="w-4 h-4 text-zinc-700 absolute left-4 top-1/2 -translate-y-1/2" /></div></div></div></div>
              </div>
            </section>
            <section className="bg-[#1f1f1f] border border-zinc-800/80 rounded-3xl p-6 shadow-xl text-left"><h3 className="text-lg font-bold text-white mb-5 flex items-center gap-2"><MapPin className="w-5 h-5 text-[#ff5351]"/> Endereço</h3><div className="grid grid-cols-1 md:grid-cols-4 gap-5 mb-4"><div className="space-y-2"><label className="text-[10px] uppercase font-black tracking-widest text-zinc-500">CEP</label><input type="text" value={clientForm.zipCode} onChange={e => setClientForm({...clientForm, zipCode: e.target.value})} className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-white focus:border-[#ff5351] outline-none text-sm" placeholder="00000-000"/></div><div className="space-y-2 md:col-span-3"><label className="text-[10px] uppercase font-black tracking-widest text-zinc-500 uppercase">Rua / Logradouro</label><input type="text" value={clientForm.address} onChange={e => setClientForm({...clientForm, address: e.target.value.toUpperCase()})} className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-white focus:border-[#ff5351] outline-none text-sm uppercase" placeholder="AV. PRINCIPAL"/></div></div><div className="grid grid-cols-1 md:grid-cols-4 gap-5"><div className="space-y-2"><label className="text-[10px] uppercase font-black tracking-widest text-zinc-500 uppercase">Número</label><input type="text" value={clientForm.number} onChange={e => setClientForm({...clientForm, number: e.target.value.toUpperCase()})} className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-white focus:border-[#ff5351] outline-none text-sm uppercase" placeholder="123"/></div><div className="space-y-2"><label className="text-[10px] uppercase font-black tracking-widest text-zinc-500 uppercase">Bairro</label><input type="text" value={clientForm.neighborhood} onChange={e => setClientForm({...clientForm, neighborhood: e.target.value.toUpperCase()})} className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-white focus:border-[#ff5351] outline-none text-sm uppercase" placeholder="CENTRO"/></div><div className="space-y-2"><label className="text-[10px] uppercase font-black tracking-widest text-zinc-500 uppercase">Cidade</label><input type="text" value={clientForm.city} onChange={e => setClientForm({...clientForm, city: e.target.value.toUpperCase()})} className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-white focus:border-[#ff5351] outline-none text-sm uppercase" placeholder="SÃO PAULO"/></div><div className="space-y-2"><label className="text-[10px] uppercase font-black tracking-widest text-zinc-500 uppercase">Estado</label><input type="text" value={clientForm.state} onChange={e => setClientForm({...clientForm, state: e.target.value.toUpperCase()})} className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-white focus:border-[#ff5351] outline-none text-sm uppercase" placeholder="SP"/></div></div></section>
            <section className="bg-[#1f1f1f] border border-zinc-800/80 rounded-3xl p-6 shadow-xl mb-10 text-left"><h3 className="text-lg font-bold text-white mb-5 flex items-center gap-2 uppercase italic"><Briefcase className="w-5 h-5 text-[#ff5351]"/> Dados Comerciais</h3><div className="grid grid-cols-1 md:grid-cols-2 gap-5"><div className="space-y-2"><label className="text-[10px] uppercase font-black tracking-widest text-zinc-500">Telefone / WhatsApp</label><div className="relative"><input type="text" value={clientForm.phone} onChange={e => setClientForm({...clientForm, phone: e.target.value})} className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-4 pl-11 text-white text-sm" placeholder="(00) 00000-0000"/><Phone className="w-4 h-4 text-zinc-700 absolute left-4 top-1/2 -translate-y-1/2" /></div></div><div className="space-y-2"><label className="text-[10px] uppercase font-black tracking-widest text-zinc-500 uppercase">Responsável de Contato</label><div className="relative"><input type="text" value={clientForm.responsibleContact} onChange={e => setClientForm({...clientForm, responsibleContact: e.target.value.toUpperCase()})} className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-4 pl-11 text-white focus:border-[#ff5351] outline-none text-sm uppercase" placeholder="NOME DO RESPONSÁVEL LEGAL"/><User className="w-4 h-4 text-zinc-700 absolute left-4 top-1/2 -translate-y-1/2" /></div></div></div></section>
          </div>
        </div>
      )}
        ...
        <div className="animate-in slide-in-from-bottom-8 duration-500 text-left">
          ...
          {isAddingTeamMember && (
            <form autoComplete="off" onSubmit={handleSaveTeamMember} className="space-y-6 mb-12 animate-in fade-in duration-300 text-left">
              <section className="bg-[#1f1f1f] border border-zinc-800/80 rounded-3xl p-8 shadow-xl">
                <div className="flex items-center gap-3 mb-8"><User className="w-5 h-5 text-[#ff5351]" /><h3 className="text-xl font-bold text-white uppercase italic">Informações Pessoais</h3></div>
                
                <div className="flex flex-col md:flex-row gap-10 items-start">
                  <div className="space-y-3">
                    <label className="text-[10px] uppercase font-black tracking-widest text-zinc-500 block ml-1">Foto</label>
                    <button type="button" onClick={() => memberFileInputRef.current?.click()} className="w-[110px] h-[110px] rounded-full border-2 border-dashed border-zinc-800 bg-zinc-900/50 hover:bg-zinc-900 hover:border-[#ff5351]/50 transition-all flex flex-col items-center justify-center gap-1 group overflow-hidden relative">
                      {memberForm.photoUrl ? (
                        ...
                        <>
                          ...
                          <span className="text-[8px] font-black uppercase text-zinc-500 text-center px-2">JPG/PNG<br/>2MB MÁX</span>
                        </>
                      )}
                    </button>
                    {memberForm.photoUrl && (
                       <div className="space-y-3 bg-zinc-900/50 p-3 rounded-2xl border border-zinc-800 mt-4">
                          <div className="flex flex-col gap-1">
                             <span className="text-[7px] font-black text-zinc-500 uppercase tracking-widest flex items-center gap-1"><Sliders className="w-2.5 h-2.5" /> Zoom</span>
                             <input type="range" min="1" max="3" step="0.05" value={photoConfig.zoom} onChange={e => setPhotoConfig({...photoConfig, zoom: Number(e.target.value)})} className="w-full h-1.5 accent-[#ff5351] bg-zinc-800 rounded-lg appearance-none cursor-pointer" />
                          </div>
                          <div className="flex flex-col gap-1">
                             <span className="text-[7px] font-black text-zinc-500 uppercase tracking-widest flex items-center gap-1"><Move className="w-2.5 h-2.5" /> Posição X</span>
                             <input type="range" min="0" max="100" value={photoConfig.x} onChange={e => setPhotoConfig({...photoConfig, x: Number(e.target.value)})} className="w-full h-1.5 accent-[#ff5351] bg-zinc-800 rounded-lg appearance-none cursor-pointer" />
                          </div>
                          <div className="flex flex-col gap-1">
                             <span className="text-[7px] font-black text-zinc-500 uppercase tracking-widest flex items-center gap-1"><Move className="w-2.5 h-2.5" /> Posição Y</span>
                             <input type="range" min="0" max="100" value={photoConfig.y} onChange={e => setPhotoConfig({...photoConfig, y: Number(e.target.value)})} className="w-full h-1.5 accent-[#ff5351] bg-zinc-800 rounded-lg appearance-none cursor-pointer" />
                          </div>
                       </div>
                    )}
                  ...
                  <div className="flex-1 space-y-6 w-full">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                      <div className="space-y-2 uppercase text-left"><label className="text-[10px] uppercase font-black tracking-widest text-zinc-500 block text-left">NOME</label><input type="text" value={memberForm.firstName} onChange={e => setMemberForm({...memberForm, firstName: e.target.value.toUpperCase()})} placeholder="Ex: JOÃO" className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-4 text-white focus:border-[#ff5351] outline-none text-sm uppercase font-bold"/></div>
                      <div className="space-y-2 uppercase text-left"><label className="text-[10px] uppercase font-black tracking-widest text-zinc-500 block text-left">SOBRENOME</label><input type="text" value={memberForm.lastName} onChange={e => setMemberForm({...memberForm, lastName: e.target.value.toUpperCase()})} placeholder="Ex: SILVA" className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-4 text-white focus:border-[#ff5351] outline-none text-sm uppercase font-bold"/></div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                      <div className="space-y-2 text-left"><label className="text-[10px] uppercase font-black tracking-widest text-zinc-500 block text-left">E-MAIL</label><div className="relative"><input type="email" value={memberForm.email} onChange={e => setMemberForm({...memberForm, email: e.target.value.toLowerCase().trim()})} placeholder="joao.silva@empresa.com" className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-4 pl-11 text-white focus:border-[#ff5351] outline-none text-sm font-bold"/><Mail className="w-4 h-4 text-zinc-500 absolute left-4 top-1/2 -translate-y-1/2" /></div></div>
                      <div className="space-y-2 text-left"><label className="text-[10px] uppercase font-black tracking-widest text-zinc-500 block text-left">TELEFONE</label><div className="relative"><input type="text" value={memberForm.phone} onChange={e => setMemberForm({...memberForm, phone: e.target.value})} placeholder="(00) 00000-0000" className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-4 pl-11 text-white focus:border-[#ff5351] outline-none text-sm font-bold"/><Phone className="w-4 h-4 text-zinc-500 absolute left-4 top-1/2 -translate-y-1/2" /></div></div>
                    </div>
                  </div>
                </div>
              </section>
              ...
              <section className="bg-[#1f1f1f] border border-zinc-800/80 rounded-3xl p-8 shadow-xl text-left">
                <div className="flex items-center gap-3 mb-8"><Briefcase className="w-5 h-5 text-[#ff5351]" /><h3 className="text-lg font-bold text-white uppercase italic leading-none text-left">Informações Profissionais</h3></div>
          ...
          <DataTable 
            ...
            columns={[
              { header: 'Membro', accessor: (member) => <div className="text-left py-1"><div className="text-white font-bold text-base leading-tight mb-1 uppercase">{member.name}</div><div className="text-zinc-500 text-xs flex items-center gap-2 leading-none uppercase"><Mail className="w-3.5 h-3.5" />{member.email}</div></div> },
