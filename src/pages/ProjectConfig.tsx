...
export default function ProjectConfig() {
  ...
  const startUpload = async (uploadId: string) => {
    ...
    try {
      console.log(`[Upload] 1. Solicitando URL no Worker via GET...`);
      ...
      if (!data.success || !data.result?.uploadURL) {
        console.error('[Upload] Worker sem sucesso:', data);
        throw new Error('Worker não retornou URL do Cloudflare Stream.');
      }
      
      const { uid, uploadURL } = data.result;
      console.log(`[Upload] UID: ${uid}`);
      console.log(`[Upload] URL de Destino: ${uploadURL}`);

      // Validação Crítica: A URL deve ser do Cloudflare
      if (!uploadURL.startsWith('https://upload.cloudflarestream.com')) {
        console.warn("[Upload] DIVERGÊNCIA: A URL de destino não parece ser do Cloudflare Stream padrão.");
      }

      // 2. Upload DIRETO (Binary PUT)
      const xhr = new XMLHttpRequest();
      activeRequests.current[uploadId] = xhr;

      console.log(`[Upload] 2. Iniciando PUT Binário para: ${uploadURL}`);
      xhr.open('PUT', uploadURL, true);
      // Não definimos timeout para permitir uploads longos
      xhr.timeout = 0; 
      ...
      xhr.onload = async () => {
        ...
        console.log(`[Upload] Resposta Cloudflare: ${xhr.status}`);
        ...
        if (xhr.status === 200 || xhr.status === 201 || xhr.status === 204) {
          ...
          console.error('[Upload] Detalhes do Erro:', xhr.status, xhr.responseText);
          // Se o erro for 413, é quase certo que o arquivo atingiu algum limite de infraestrutura
          const errorMsg = xhr.status === 413 
            ? "O arquivo é muito grande (Erro 413). Tente um arquivo menor ou verifique se o upload está sendo desviado para o servidor local."
            : `Erro no Cloudflare (${xhr.status})`;
          alert(errorMsg);
      ...
      xhr.onerror = () => {
        delete activeRequests.current[uploadId];
        alert('Erro de conexão ou CORS no upload direto.');
        setUploads(prev => ({ ...prev, [uploadId]: { ...prev[uploadId], status: 'error' } }));
      };

      // Envia o BINÁRIO PURO do arquivo
