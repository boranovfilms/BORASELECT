...
import { ArrowLeft, Plus, Loader2, Save, Trash2, X, Search, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function OrcamentoDetalhe() {
      ...
      try {
        ...
        if (!isNovo && id) {
          ...
          const numerosBase = new Set(
            orcSnap.docs.map(d => (d.data().numero as string) || '').map(n => n.split('-v')[0])
          );
  ...
  const inputClass = (campo: string) => cn(
    "w-full h-11 bg-zinc-900 border rounded-xl px-4 text-white text-sm focus:border-[#ff5351] outline-none",
    camposComErro.includes(campo) ? 'border-red-500' : 'border-zinc-800'
  );
      ...
      <header className="flex items-start justify-between gap-4">
        ...
        {!somenteLeitura && (
          <button onClick={handleSalvar} disabled={salvando}
            className="h-10 px-6 bg-[#ff5351] text-white rounded-xl font-black uppercase text-[9px] tracking-widest hover:brightness-110 transition-all flex items-center gap-2 disabled:opacity-50 shrink-0 mt-8 shadow-xl shadow-[#ff5351]/20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" value={form.localEvento || ''} onChange={e => updateForm({ localEvento: e.target.value })}
                  ...
                  className={inputClass('localEvento')} />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="date" value={form.dataEventoInicio || ''} onChange={e => updateForm({ dataEventoInicio: e.target.value })}
                  readOnly={somenteLeitura}
                  className={inputClass('dataEventoInicio')} />
              </div>
              <div>
