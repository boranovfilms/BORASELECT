...
export default function OrcamentoEquipamentos() {
  ...
  const importarDaPlanilha = async () => {
    ...
    try {
      ...
      toast.success(`${itens.length} equipamentos importados com sucesso!`);
    } catch {
    ...
    <div className="space-y-8 pb-20 text-left">
      <header className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-500 text-sm mt-1">Tabela de custos internos — visível apenas para direção</p>
        </div>
        <div className="flex gap-2 shrink-0">
          ...
          <button
            onClick={() => { setShowForm(true); setEditando(null); setForm({ nome: '', categoria: 'Câmeras', valorDia: 0, ativo: true }); }}
        ...
        <div className="text-center py-20 text-zinc-600">
          ...
          {equipamentos.length === 0 && (
            <p className="text-xs mt-2 text-zinc-700">
              Clique em "Importar Planilha" para carregar os itens da sua planilha de uma vez
            </p>
          )}
        </div>
      ) : (
