  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      const filtered = allTasks.filter(task => {
        ...
        // ✅ Somente tarefas pessoais — sem tipo de planejamento
        if (task.tipo && ['validacao_planejamento', 'planejamento_validado_equipe', 'planejamento_revisado', 'tarefa_departamento'].includes(task.tipo)) return false;
    ...
    <div className="animate-in fade-in duration-700 pb-20">
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
        <div>
          ...
          <p className="text-zinc-500 text-sm mt-2">Bloco de notas pessoal — anotações do dia a dia.</p>
        </div>
          ...
          <div className="bg-[#141414] border border-zinc-800 rounded-[32px] p-8 shadow-2xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              <div className="space-y-2 relative">
                ...
                <div className="relative">
                  <select value={newTask.delegadoPara} onChange={e => setNewTask({...newTask, delegadoPara: e.target.value})} className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-4 text-sm text-white focus:border-[#ff5351] outline-none appearance-none cursor-pointer">
                    <option value="">NÃO DELEGAR (PARTICULAR)</option>
            ...
            <div className="flex justify-end pt-8 border-t border-zinc-800 mt-8">
              <button type="submit" disabled={saving} className="h-14 px-10 bg-white text-black rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-[#ff5351] hover:text-white transition-all flex items-center gap-3 shadow-2xl">
                {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />} Salvar Atividade
              </button>
            </div>
          </div>
        </form>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-zinc-800 mb-6 pb-4">
        <div className="flex gap-6">
          {['pendentes', 'executadas'].map((tab) => (
            <button key={tab} onClick={() => { setActiveTab(tab as any); setSelectedTasks(new Set()); }} className={cn("text-[10px] uppercase font-black tracking-widest pb-2 border-b-2 transition-all", activeTab === tab ? 'text-white border-[#ff5351]' : 'text-zinc-500 border-transparent')}>
              {tab}
            </button>
          ))}
        </div>
        {activeTab === 'pendentes' && selectedTasks.size > 0 && (
          <button onClick={handleCompleteSelectedTasks} disabled={saving} className="px-6 py-2.5 bg-emerald-500 text-white rounded-xl font-black uppercase tracking-widest text-[10px] hover:bg-emerald-600 transition-all flex items-center gap-2 shadow-lg shadow-emerald-500/20 animate-in zoom-in-95">
            {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />} Finalizar Selecionadas ({selectedTasks.size})
          </button>
        )}
      </div>
            ...
            accessor: (task: Task) => (
              <div className="flex justify-center" onClick={(e) => e.stopPropagation()}>
                <button
                  ...
                  className={cn("w-5 h-5 rounded border-2 transition-all flex items-center justify-center shrink-0", selectedTasks.has(task.id!) ? "bg-[#ff5351] border-[#ff5351]" : "border-zinc-700 bg-zinc-900 hover:border-zinc-500")}
                >
