...
export default function Dashboard() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'Em Seleção':
        return 'text-[#ff5351] border-[#ff5351] bg-[#ff5351]/10';
      case 'Finalizado':
        return 'text-zinc-400 border-zinc-700 bg-zinc-800';
      case 'Aguardando Cliente':
        return 'text-cyan-400 border-cyan-800 bg-cyan-900/10';
      default:
    ...
    <div className="space-y-12">
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
          {[1, 2, 3].map((item) => (
            <div key={item} className="aspect-[4/3] bg-zinc-900/50 rounded-2xl animate-pulse border border-zinc-800" />
          ))}
        </div>
          ...
          {isAdminUser && adminProjects.length > 0 && (
            <div className="space-y-6">
              {clientProjects.length > 0 && (
                <h2 className="text-sm font-black uppercase tracking-[0.3em] text-zinc-600 border-b border-zinc-800 pb-4">
                  Gerenciados por mim
                </h2>
              )}
                    ...
                    <div className="p-6 flex flex-col flex-1">
                      <div className="mb-6">
                        ...
                        <p className="text-zinc-500 text-sm">
                          Cliente: {project.clientName || project.clientEmail || 'SEM CLIENTE'}
                        </p>
                      </div>
