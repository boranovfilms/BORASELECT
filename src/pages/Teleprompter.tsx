...
import { 
  ...
  Smartphone, Laptop, MoveHorizontal, Type, Upload, X, Minus, Plus, FileText, ChevronRight, ChevronLeft
} from 'lucide-react';
...
export default function Teleprompter() {
    ...
    const unsubscribe = onSnapshot(docRef, (docSnap) => {
      if (docSnap.exists()) {
        ...
        if (data.posicao !== undefined && prompterRef.current && selectedMode === 'prompter') {
          ...
          if (Math.abs(scrollPos.current - targetScroll) > 20) {
    ...
    return (
      <div className="space-y-1 w-full select-none text-left">
        ...
        <div className="flex items-center gap-2">
          <button type="button" onClick={() => onChange(Math.max(min, value - 1))} className="w-8 h-8 rounded-full bg-zinc-800 border border-zinc-700 flex items-center justify-center text-white active:bg-[#ff5351] transition-all shrink-0"><Minus className="w-4 h-4" /></button>
          <div className="relative h-[27px] flex-1 flex items-center">
            <input type="range" min={min} max={max} step="1" value={value} onChange={e => onChange(Number(e.target.value))} className="pill-range-input" style={{ background: `linear-gradient(to right, #ff5351 ${percentage}%, #1f1f1f ${percentage}%)` } as any} />
          </div>
          <button type="button" onClick={() => onChange(Math.min(max, value + 1))} className="w-8 h-8 rounded-full bg-zinc-800 border border-zinc-700 flex items-center justify-center text-white active:bg-[#ff5351] transition-all shrink-0"><Plus className="w-4 h-4" /></button>
        </div>
  ...
  if (selectedMode === 'remote') {
    return (
      <div className="animate-in fade-in duration-700 min-h-[85vh] flex flex-col gap-6 max-w-lg mx-auto pb-10 px-4 max-w-[100vw] overflow-x-hidden">
        ...
        {remoteTab === 'control' ? (
          <div className="space-y-6 animate-in slide-in-from-bottom-4">
            ...
            <div className="flex gap-4 items-stretch">
              ...
              <div className="w-[80px] bg-[#141414] border border-zinc-800 rounded-[32px] shadow-xl flex flex-col items-center justify-between py-6">
                <span className="text-[8px] font-black text-zinc-500 uppercase tracking-widest">POS</span>
                <div className="flex flex-col gap-4">
                  <button onClick={() => updateState({ posicao: Math.min(100, (state.posicao || 0) + 10) })} className="w-14 h-14 rounded-full bg-zinc-900 border border-zinc-800 flex items-center justify-center text-white active:bg-[#ff5351] shadow-lg transition-all"><ChevronUp className="w-6 h-6" /></button>
                  <button onClick={() => updateState({ posicao: Math.max(0, (state.posicao || 0) - 10) })} className="w-14 h-14 rounded-full bg-zinc-900 border border-zinc-800 flex items-center justify-center text-white active:bg-[#ff5351] shadow-lg transition-all"><ChevronDown className="w-6 h-6" /></button>
                </div>
                <span className="text-[10px] font-black text-white italic">{Math.round(state.posicao || 0)}%</span>
              </div>
            </div>
          ...
          <div className="flex-1 flex flex-col gap-4 animate-in slide-in-from-bottom-4 text-left">
            ...
            <div className="grid grid-cols-2 gap-3 pb-12 text-left">
  ...
  return (
    <div className="fixed inset-0 z-[500] bg-black flex flex-col overflow-hidden select-none max-w-[100vw] overflow-x-hidden" onClick={handleScreenTouch}>
      <style>{`
        .pill-range-input {
      ...
      <div className={cn(
        ...
        <div className="flex-1 grid grid-cols-3 gap-6 text-left">
          ...
          <PillSlider label="Margem" value={state.margem} min={0} max={40} onChange={(v:any) => updateState({ margem: v })} subLabels={{ left: 'Estreito', right: 'Largo' }} />
          <PillSlider label="Fonte" value={state.fonte} min={20} max={72} onChange={(v:any) => updateState({ fonte: v })} subLabels={{ left: 'A', right: 'A Grande' }} />
        </div>
