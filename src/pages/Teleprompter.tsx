  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(docRef, (docSnap) => {
      if (docSnap.exists()) {
        ...
        // Se a posição mudou remotamente e não estamos dando play localmente
        if (data.posicao !== undefined && prompterRef.current && selectedMode === 'prompter') {
          ...
          // Só atualiza se a diferença for significativa ou se o play estiver desligado
          if (!state.playing || Math.abs(scrollPos.current - targetScroll) > 50) {
  ...
  useEffect(() => {
    ...
    const scroll = () => {
      if (state.playing && prompterRef.current && selectedMode === 'prompter') {
        ...
        // Atualiza a posição percentual no banco a cada 2 segundos (throttled)
        // para não sobrecarregar, mas manter o controle ciente
      }
    ...
    return (
      <div className="space-y-1 w-full select-none text-left">
        ...
        <div className="flex items-center gap-2">
          <button type="button" onClick={(e) => { e.stopPropagation(); onChange(Math.max(min, value - 1)); }} className="w-7 h-7 shrink-0 rounded-full bg-zinc-800 border border-zinc-700 flex items-center justify-center text-white hover:text-[#ff5351] hover:border-[#ff5351] transition-all"><Minus className="w-3.5 h-3.5" /></button>
          <div className="relative h-[27px] flex-1 flex items-center group"><input type="range" min={min} max={max} step="0.1" value={value} onChange={e => onChange(Number(e.target.value))} className="pill-range-input" style={{ background: `linear-gradient(to right, #ff5351 ${percentage}%, #1f1f1f ${percentage}%)` } as any} /></div>
          <button type="button" onClick={(e) => { e.stopPropagation(); onChange(Math.min(max, value + 1)); }} className="w-7 h-7 shrink-0 rounded-full bg-zinc-800 border border-zinc-700 flex items-center justify-center text-white hover:text-[#ff5351] hover:border-[#ff5351] transition-all"><Plus className="w-3.5 h-3.5" /></button>
        </div>
        {subLabels && (<div className="flex justify-between px-9 text-[7px] font-black text-zinc-600 uppercase tracking-tighter"><span>{subLabels.left}</span><span>{subLabels.right}</span></div>)}
      </div>
    );
  };
    ...
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center gap-10 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-2xl">
          <button onClick={() => { setSelectedMode('prompter'); sessionStorage.setItem('tp_selected_mode', 'prompter'); }} className="p-10 bg-[#141414] border-2 border-zinc-800 rounded-[40px] hover:border-[#ff5351] transition-all flex flex-col items-center gap-4 shadow-2xl">
            <Tv className="w-12 h-12 text-[#ff5351]" />
            <span className="text-lg font-black text-white uppercase italic">Modo Teleprompter</span>
          </button>
          <button onClick={() => { setSelectedMode('remote'); sessionStorage.setItem('tp_selected_mode', 'remote'); }} className="p-10 bg-[#141414] border-2 border-zinc-800 rounded-[40px] hover:border-[#ff5351] transition-all flex flex-col items-center gap-4 shadow-2xl">
            <Smartphone className="w-12 h-12 text-[#ff5351]" />
  ...
  if (selectedMode === 'remote') {
    return (
      <div className="min-h-[85vh] flex flex-col gap-6 max-w-lg mx-auto p-4 text-left pb-12">
        <header className="bg-[#141414] border border-zinc-800 p-6 rounded-[32px] flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div><h2 className="text-white font-black uppercase italic leading-none">Boranov TP</h2><span className="text-[8px] font-black text-emerald-500 uppercase tracking-widest animate-pulse">Nuvem Online</span></div>
          </div>
        ...
        {remoteTab === 'control' ? (
          <div className="space-y-6 animate-in slide-in-from-bottom-4">
            ...
            <div className="flex gap-4 items-stretch">
              <div className="flex-1 space-y-4">
                <div className="bg-[#141414] border border-zinc-800 p-5 rounded-[32px] shadow-xl">
                  <PillSlider label="Velocidade" value={state.velocidade} min={1} max={20} onChange={(v:any) => updateState({ velocidade: v })} subLabels={{ left: 'Lento', right: 'Rápido' }} />
                </div>
              ...
              <div className="w-[80px] bg-[#141414] border border-zinc-800 rounded-[32px] shadow-xl flex flex-col items-center py-6 px-2">
                ...
                <div className="flex-1 w-full relative group">
                  <input 
                    ...
                    min="0" max="100" 
                    step="1"
                    value={state.posicao || 0} 
                    onChange={e => updateState({ posicao: Number(e.target.value) })}
                    className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[180px] accent-[#ff5351] cursor-pointer" 
                    style={{ 
                      appearance: 'none', 
                  ...
                  />
                  <div className="absolute inset-0 pointer-events-none flex flex-col justify-between py-2 text-[7px] font-black text-zinc-700 uppercase">
                    <span>FIM</span>
                    <span>INI</span>
                  </div>
                </div>
          ...
          <div className="flex-1 flex flex-col gap-4 min-h-[400px] animate-in slide-in-from-bottom-4">
            ...
            <div className="grid grid-cols-2 gap-4 pb-12">
              ...
              <button onClick={clearText} className="py-5 bg-zinc-800/50 rounded-2xl flex items-center justify-center gap-2 text-zinc-500 font-black uppercase text-[10px] hover:text-red-500 transition-all"><Trash2 className="w-4 h-4" /> Limpar</button>
            </div>
          </div>
        )}
      </div>
      ...
      <div className={cn("fixed bottom-8 left-1/2 -translate-x-1/2 z-[400] w-[95%] max-w-5xl bg-zinc-900/95 backdrop-blur-3xl border border-white/5 rounded-[40px] p-4 flex items-center gap-6 shadow-2xl transition-all duration-700", !showControls && "opacity-0 translate-y-32")}>
        ...
        <div className="flex items-center gap-3 pr-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => { setSelectedMode(null); sessionStorage.removeItem('tp_selected_mode'); }} className="p-4 bg-zinc-900 border border-zinc-800 text-zinc-600 hover:text-white rounded-2xl transition-all"><Laptop className="w-5 h-5" /></button>
          </div>
        </div>
      </div>
    </div>
