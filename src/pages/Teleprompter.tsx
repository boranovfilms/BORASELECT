...
export default function Teleprompter() {
    ...
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center gap-10 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-2xl">
          <button onClick={() => { setSelectedMode('prompter'); sessionStorage.setItem('tp_selected_mode', 'prompter'); }} className="p-10 bg-[#141414] border-2 border-zinc-800 rounded-[40px] hover:border-[#ff5351] transition-all flex flex-col items-center gap-4">
            <Tv className="w-12 h-12 text-[#ff5351]" />
            <span className="text-lg font-black text-white uppercase italic">Modo Teleprompter</span>
          </button>
          <button onClick={() => { setSelectedMode('remote'); sessionStorage.setItem('tp_selected_mode', 'remote'); }} className="p-10 bg-[#141414] border-2 border-zinc-800 rounded-[40px] hover:border-[#ff5351] transition-all flex flex-col items-center gap-4">
  ...
  if (selectedMode === 'remote') {
    return (
      <div className="min-h-[85vh] flex flex-col gap-6 max-w-lg mx-auto p-4 text-left">
          ...
          <div className="space-y-6">
            ...
            {[
              ...
              <div key={ctrl.id} className="bg-[#141414] border border-zinc-800 p-6 rounded-[32px]">
                <div className="flex justify-between items-center mb-4"><span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">{ctrl.label}</span><span className="text-xs font-black text-white">{ctrl.value}</span></div>
                <div className="flex items-center gap-3">
                  ...
                  <input type="range" min={ctrl.min} max={ctrl.max} step="1" value={ctrl.value} onChange={e => updateState({ [ctrl.id]: Number(e.target.value) })} className="flex-1 h-2 rounded-lg accent-[#ff5351] bg-zinc-800" />
            ...
            <div className="grid grid-cols-2 gap-4">
              <button onClick={() => updateState({ voltarInicio: true })} className="h-20 bg-zinc-900 border border-zinc-800 rounded-[24px] flex flex-col items-center justify-center gap-1 text-zinc-400 active:bg-zinc-800">
                <RotateCcw className="w-5 h-5" /><span className="text-[9px] font-black uppercase tracking-widest">Início</span>
              </button>
              <button onClick={() => updateState({ espelhado: !state.espelhado })} className={cn("h-20 border rounded-[24px] flex flex-col items-center justify-center gap-1 transition-all", state.espelhado ? "bg-[#ff5351] border-[#ff5351] text-white" : "bg-zinc-900 border-zinc-800 text-zinc-400")}>
                <FlipHorizontal className="w-5 h-5" /><span className="text-[9px] font-black uppercase tracking-widest">Espelhar</span>
              </button>
            </div>
          </div>
        ) : (
          <div className="flex-1 flex flex-col gap-4 min-h-[400px]">
            <textarea value={state.texto} onChange={e => updateState({ texto: e.target.value })} placeholder="DIGITE O ROTEIRO..." className="flex-1 bg-black border border-zinc-800 rounded-[32px] p-6 text-sm text-white resize-none outline-none focus:border-[#ff5351]" />
            <div className="grid grid-cols-2 gap-4 pb-8">
              <label className="flex-1 py-5 bg-zinc-800 rounded-2xl flex items-center justify-center gap-2 cursor-pointer text-white font-black uppercase text-[10px]">
                <Upload className="w-4 h-4" /> Arquivo .TXT
                <input type="file" accept=".txt" onChange={handleFileUpload} className="hidden" />
              </label>
              <button onClick={() => { if(window.confirm('Limpar texto?')) updateState({ texto: '' }); }} className="py-5 bg-zinc-800/50 rounded-2xl flex items-center justify-center gap-2 text-zinc-500 font-black uppercase text-[10px]"><Trash2 className="w-4 h-4" /> Limpar</button>
            </div>
          </div>
        )}
      </div>
      ...
      <div className={cn("fixed bottom-8 left-1/2 -translate-x-1/2 z-[400] w-[95%] max-w-5xl bg-zinc-900/95 backdrop-blur-3xl border border-white/5 rounded-[40px] p-4 flex items-center gap-6 shadow-2xl transition-all duration-700", !showControls && "opacity-0 translate-y-32")}>
        ...
        <div className="flex-1 grid grid-cols-3 gap-6">
          {[
            ...
            <div key={ctrl.id} className="flex flex-col gap-1 items-start">
              <span className="text-[8px] font-black text-zinc-500 uppercase ml-1">{ctrl.id} ({ctrl.value})</span>
              <input type="range" min={ctrl.min} max={ctrl.max} value={ctrl.value} onChange={e => updateState({ [ctrl.id]: Number(e.target.value) })} className="w-full h-2 rounded-lg accent-[#ff5351] bg-zinc-800" />
            </div>
          ))}
        </div>
        ...
        <div className="flex items-center gap-3 pr-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => { setSelectedMode(null); sessionStorage.removeItem('tp_selected_mode'); }} className="p-4 bg-zinc-800 text-zinc-600 hover:text-white rounded-2xl transition-all"><Laptop className="w-5 h-5" /></button>
          </div>
        </div>
      </div>
    </div>
