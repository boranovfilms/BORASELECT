...
export default function ProjectReview() {
  ...
  // Novos estados para o fluxo de PIX
  const [submittingCreditRequest, setSubmittingCreditRequest] = useState(false);
  const [submittingPixRequest, setSubmittingPixRequest] = useState(false);
  const [pixData, setPixData] = useState<{ txid: string; copiaECola: string; qrCodeUrl: string } | null>(null);
  const [pixPaid, setPixPaid] = useState(false); // NOVO ESTADO: Controla a tela verde

  const loadingTimeoutRef = useRef<NodeJS.Timeout | null>(null);
  // Ref para guardar o total de créditos inicial e saber quando atualizou
  ...
  useEffect(() => {
    if (project) {
      ...
      if (initialCreditsRef.current === null) {
        initialCreditsRef.current = project.creditsTotal || 0;
      }
    }
  }, [project?.id, project?.extraPrice]);

  // 🟢 Polling ativo (a cada 5 segundos) AGORA ENVIA O ID DO PROJETO E CRÉDITOS NA URL
  useEffect(() => {
    if (!id || pageMode !== 'credits' || !pixData || !pixData.txid || pixPaid) return;

    const intervalId = setInterval(async () => {
      try {
        // Envia o txid, o id do projeto e a quantidade de creditos direto para API!
        const res = await fetch(`/api/pix-check-status?txid=${pixData.txid}&projectId=${id}&credits=${creditsToBuy}`);
        if (res.ok) {
          const data = await res.json();
          if (data.paid) {
            clearInterval(intervalId); // Para de perguntar
            
            // ATUALIZA A TELA PARA VERDE
            setPixPaid(true); 
            toast.success('Pagamento confirmado pelo Sicredi!');
            
            // Aguarda 4 segundos com a tela verde antes de voltar para os vídeos
            setTimeout(async () => {
              const updatedProject = await projectService.getProject(id);
              if (updatedProject) {
                setProject(updatedProject);
                initialCreditsRef.current = updatedProject.creditsTotal || 0;
              }
        ...
        console.error('Erro ao verificar status do PIX via polling:', error);
      }
    }, 5000); // 5 segundos

    return () => clearInterval(intervalId);
  }, [id, pageMode, pixData, pixPaid, creditsToBuy]);
  ...
  const handleCopyPixKey = async (textToCopy: string) => {
    try {
  ...
  const handleGeneratePixPayment = async () => {
    ...
    try {
      setSubmittingPixRequest(true);
      const response = await fetch('/api/pix-create-charge', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          valor: creditRequestTotal,
          infoAdicional: `Créditos: ${project.title}`
        })
      });
      ...
      if (!response.ok || !data.success) {
        throw new Error(data.error || 'Erro ao gerar cobrança no banco');
      }
      ...
      fetch('/api-v2/credits/request', {
        ...
        body: JSON.stringify({
          ...
          clientNote: `TXID do Banco: ${data.txid} - ${creditClientNote}`
        })
      }).catch(() => {});

      toast.success('Cobrança PIX gerada com sucesso!');
    } catch (error: any) {
      toast.error(error.message || 'Erro ao comunicar com o banco Sicredi');
    } finally {
      setSubmittingPixRequest(false);
    }
  };
          ...
          <section className="space-y-5">
            <div className="rounded-3xl border border-zinc-800 bg-[#141414] p-5 md:p-6 space-y-5">
              <div className="flex items-end justify-between gap-4">
                <div>
                  <p className="text-[10px] uppercase tracking-[0.25em] text-zinc-500 font-black mb-2">Quantidade</p>
                  <h2 className="text-white text-xl md:text-2xl font-black uppercase">Créditos desejados</h2>
                </div>
              ...
              <div className="flex items-center gap-3">
                <button
                  type="button"
                  disabled={pixData !== null}
                  onClick={() => setCreditsToBuy(prev => Math.max(1, prev - 1))}
                  className="w-11 h-11 rounded-2xl border border-zinc-800 bg-zinc-900 text-white font-black text-xl hover:border-zinc-700 transition-all shrink-0 disabled:opacity-50"
                >
                  -
                </button>
                <input
                  ...
                  className="flex-1 bg-zinc-900 border border-zinc-800 rounded-2xl px-4 py-3 text-white text-center text-2xl font-black focus:border-[#ff5351] outline-none transition-all disabled:opacity-50"
                />
                <button
                  type="button"
                  disabled={pixData !== null}
              ...
              <div className="grid grid-cols-3 gap-3">
                {[5, 10, 20].map((amount) => (
                  <button
                    ...
                    className={cn(
                      'h-11 rounded-2xl border text-xs md:text-sm font-black uppercase tracking-wide transition-all disabled:opacity-50',
                      creditsToBuy === amount
            ...
            <div className="rounded-3xl border border-zinc-800 bg-[#141414] p-5 md:p-6 space-y-4">
              <div>
                <p className="text-[10px] uppercase tracking-[0.25em] text-zinc-500 font-black mb-2">PAGAMENTO AUTOMÁTICO</p>
                <h2 className="text-white text-xl md:text-2xl font-black uppercase">Finalizar Compra</h2>
              </div>
              
              {!pixData ? (
                <>
                  <div className="rounded-2xl border border-zinc-800 bg-zinc-900/70 p-4 text-sm text-zinc-300 leading-relaxed mb-4">
                    Ao gerar o pagamento, um QR Code exclusivo será criado para esta solicitação. Assim que você pagar pelo app do seu banco, <span className="text-[#ff5351] font-black">os créditos serão liberados automaticamente</span> no seu projeto.
                  </div>
                  
                  <button 
                    type="button"
                    onClick={handleGeneratePixPayment}
                    disabled={submittingPixRequest || effectiveCreditUnitPrice <= 0}
                    className="w-full h-14 rounded-2xl bg-[#ff5351] text-white font-black uppercase tracking-widest text-sm hover:brightness-110 transition-all shadow-2xl shadow-[#ff5351]/20 disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-3"
                  >
                    {submittingPixRequest ? (
                      <><Loader2 className="w-5 h-5 animate-spin" /> GERANDO COBRANÇA PIX...</>
                    ) : (
                      <><QrCode className="w-5 h-5" /> GERAR QR CODE PIX - {formatCurrency(creditRequestTotal)}</>
                    )}
                  </button>
                </>
              ) : pixPaid ? (
                ...
                <div className="space-y-6 animate-in fade-in zoom-in duration-500">
                  <div className="flex flex-col items-center justify-center gap-4 p-8 rounded-2xl bg-[#22c55e]/10 border border-[#22c55e]/30 relative overflow-hidden">
                    <div className="absolute inset-0 bg-[#22c55e]/5 animate-pulse" />
                    
                    <div className="w-20 h-20 bg-[#22c55e]/20 rounded-full flex items-center justify-center relative z-10 shadow-[0_0_30px_rgba(34,197,94,0.4)]">
                      <CheckCircle2 className="w-10 h-10 text-[#22c55e]" />
                    </div>
                    
                    <div className="relative z-10 text-center">
                      <h3 className="text-[#22c55e] font-black uppercase text-2xl mb-1">Pagamento Confirmado!</h3>
                      <p className="text-zinc-300 text-xs uppercase tracking-widest font-bold">Os créditos foram liberados e estão na sua conta.</p>
                    </div>
                  </div>
                </div>
              ) : (
                // TELA VERMELHA AGUARDANDO PAGAMENTO
                <div className="space-y-6 animate-in fade-in zoom-in duration-500">
                  <div className="flex flex-col md:flex-row items-center gap-6 p-4 rounded-2xl bg-white/5 border border-[#ff5351]/30 relative overflow-hidden">
                    <div className="absolute inset-0 bg-[#ff5351]/5 animate-pulse" />
                    
                    {/* Imagem do QR Code */}
                    <div className="w-48 h-48 bg-white rounded-xl p-2 shrink-0 relative z-10 shadow-lg shadow-black/50">
                      <img src={pixData.qrCodeUrl} alt="QR Code PIX" className="w-full h-full object-contain" />
                    </div>
                    
                    <div className="flex-1 space-y-4 relative z-10 text-center md:text-left">
                      <div>
                        <h3 className="text-white font-black uppercase text-xl mb-1">Aguardando Pagamento</h3>
                        <p className="text-zinc-400 text-xs uppercase tracking-widest">Escaneie o QR Code com o app do seu banco.</p>
                      </div>
                      
                      <div className="space-y-2">
                         <button type="button" onClick={() => handleCopyPixKey(pixData.copiaECola)} className="w-full h-12 bg-white/10 hover:bg-white/20 text-white border border-white/10 rounded-xl font-black uppercase tracking-widest text-[10px] transition-all flex items-center justify-center gap-2">
                  ...
                  <div className="text-center space-y-2">
                    <Loader2 className="w-6 h-6 text-[#ff5351] animate-spin mx-auto opacity-70" />
                    <p className="text-xs text-zinc-500 uppercase tracking-widest font-black">Assim que o pagamento for concluído,<br/>os créditos entrarão automaticamente.</p>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="rounded-3xl border border-zinc-800 bg-[#141414] p-5 md:p-6 space-y-4">
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  rows={7}
                  placeholder="Se quiser, escreva uma observação sobre o seu projeto ou seleção."
