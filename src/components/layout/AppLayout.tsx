...
export default function AppLayout({ children, userRole = 'cliente', permissions = {} }: AppLayoutProps) {
  ...
  // Lista de todos os módulos com rotas e ícones
  const ALL_MODULES = [
    { id: 'dashboard', icon: LayoutDashboard, label: 'Dashboard', path: '/' },
    { id: 'projetos', icon: Library, label: 'Projeto Seleção', path: '/projetos' }, // RENOMEADO
  ...
  // Filtra os itens do menu baseados no userRole e permissions
  const navItems = ALL_MODULES.filter(mod => {
    // Forçar exibição do item Planejamentos para Clientes
    if (mod.id === 'planejamentos' && userRole === 'cliente') return true;
    
    if (mod.id === 'painel_master' && userRole === 'master') return true;
    
    // Verifica se a matriz libera o módulo para a role do usuário
    return permissions[mod.id]?.[userRole] === true;
  });

  // Garante que o dashboard apareça pelo menos se não houver nada
    ...
    <div className="min-h-screen bg-[#131313] text-[#e2e2e2] font-sans selection:bg-[#ff5351]/30 selection:text-white">
      ...
      {showGlobalSettings && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-md bg-[#222222] rounded-[40px] border border-zinc-700 shadow-[0_0_50px_rgba(0,0,0,0.5)] overflow-hidden animate-in zoom-in-95 duration-200">
          ...
          <div className="mb-8 px-2 flex items-center gap-4">
            ...
            <div>
              ...
              <div className="text-[10px] uppercase tracking-widest text-zinc-500">
                {location.pathname.includes('/review/') ? currentProjectEmail || 'Modo Visualização' : `Logado como ${userRole}`}
              </div>
            </div>
          </div>
