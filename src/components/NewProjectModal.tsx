...
import { Check, Search, Loader2, ChevronLeft, FolderOpen, GitBranch, ChevronDown } from 'lucide-react';
...
export default function NewProjectModal({ isOpen, onClose, onSuccess }: NewProjectModalProps) {
  ...
  const loadInitialData = async () => {
    ...
    try {
      const [cls, cats, catalogs, models, team] = await Promise.all([
  ...
  const handleModelSelect = (modelId: string) => {
    ...
    if (model) {
      ...
      const initialStages: ProjectStage[] = stages.map(stage => ({
        ...
        assignee: stage?.assignee || 'Cliente', 
                ...
                <div className="space-y-3">
                  {workflowStages.map((stage, idx) => (
                    <div key={stage.id} className="flex flex-col md:flex-row md:items-center gap-3 bg-zinc-900 p-3 rounded-2xl border border-zinc-800">
                      ...
                      {/* CAMPO DE RESPONSÁVEL (SELEÇÃO DROP-DOWN) */}
                      <div className="w-full md:w-48 shrink-0">
                        <div className="relative">
                          <select 
                            value={stage.assignee} 
                            onChange={(e) => updateStageField(idx, 'assignee', e.target.value)}
                            className="w-full bg-[#111] border border-zinc-800 rounded-xl h-10 px-3 pr-8 text-xs text-white focus:border-[#ff5351] outline-none appearance-none cursor-pointer"
                          >
                            ...
                            <optgroup label="Cliente">
                              <option value="Cliente">Ação do Cliente</option>
                            </optgroup>
                          </select>
